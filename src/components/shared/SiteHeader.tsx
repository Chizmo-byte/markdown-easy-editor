...
export function SiteHeader({ current }: { current?: "editor" | "about" | "privacy" | "terms" | "contact" }) {
  return (
    <header className="border-b border-[#d8d5d2] bg-[#f3f2f2] text-[#201e1d] dark:border-zinc-700 dark:bg-[#201e1d] dark:text-[#f3f2f2]">
      <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        <Link href="/" className="group flex shrink-0 items-center gap-2" aria-label="LifeMarginトップへ">
          <span className="relative inline-flex h-8 w-8 items-center justify-center overflow-hidden bg-[#201e1d] font-serif text-sm font-bold text-[#f3f2f2] dark:bg-[#f3f2f2] dark:text-[#201e1d]">
          ...
          <span>
            <span className="block text-[10px] font-medium uppercase tracking-[0.28em] text-[#6f6a66] dark:text-[#a39d98]">LifeMargin</span>
            <span className="block font-serif text-sm font-bold tracking-tight group-hover:text-[#0088b0]">余白のある道具</span>
          </span>
        </Link>
        <nav aria-label="サイト内ナビゲーション" className="flex items-center gap-3 overflow-x-auto text-xs text-[#6f6a66] sm:gap-5 dark:text-[#a39d98]">
